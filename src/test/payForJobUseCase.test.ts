...
describe("payForJobUseCase", () => {
  let mockJobRepository: MockJobRepository;
  ...
  it("should pay for a job", async () => {
    ...
    const result = await payForJobUseCase(mockJobRepository as any, 1, 1);
  ...
  it("should not pay for a non-existing job", async () => {
    ...
    await expect(
      payForJobUseCase(mockJobRepository as any, 1, 1)
  ...
  it("should not pay for a job if it has already been paid", async () => {
    ...
    await expect(
      payForJobUseCase(mockJobRepository as any, 1, 1)
    ).rejects.toThrow("Job has already been paid");
