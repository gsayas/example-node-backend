...
describe("bestProfessionUseCase", () => {
  let mockProfessionRepository: MockProfessionRepository;
  ...
  it("should return the profession with the highest earnings", async () => {
    ...
    const result = await bestProfessionUseCase(
      mockProfessionRepository as any,
      new Date("2022-01-01"),
      new Date("2023-01-01")
    );

    expect(result.profession).toEqual("Electrician");
    expect(mockProfessionRepository.getBestProfessions).toHaveBeenCalledWith(
      new Date("2022-01-01"),
      new Date("2023-01-01")
    );
  });
