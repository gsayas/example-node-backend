...
describe("depositToBalanceUseCase", () => {
  let mockBalanceRepository: MockBalanceRepository;
  ...
  it("should deposit to user balance", async () => {
    ...
    const result = await depositToBalanceUseCase(
  ...
  it("should not deposit if the deposit amount exceeds 25% of total jobs to pay", async () => {
    ...
    await expect(
      depositToBalanceUseCase(mockBalanceRepository as any, 1, 200)
    ).rejects.toThrow("Can't deposit more than 25% of total jobs to pay");
  ...
  it("should not deposit for a non-existing user", async () => {
    ...
    await expect(
      depositToBalanceUseCase(mockBalanceRepository as any, 1, 50)
